...
- [Weather Stations Monitoring](#weather-stations-monitoring)
  - [A. Write Weather Station Mock](#a-write-weather-station-mock)
  - [B. Setup Weather Station to Connect to Kafka](#b-setup-weather-station-to-connect-to-kafka)
  - [C. Implement Raining Triggers in Kafka Processors](#c-implement-raining-triggers-in-kafka-processors)
  - [D. Implement Central Station](#d-implement-central-station)
    - [BitCask Riak to store updated view of weather status](#bitcask-riak-to-store-updated-view-of-weather-status)
    - [Archiving of all Weather Statuses in Parquet Files](#archiving-of-all-weather-statuses-in-parquet-files)
  - [E. Setup Historical Weather Statuses Analysis](#e-setup-historical-weather-statuses-analysis)
    - [Steps](#steps)
    - [Analysis Results](#analysis-results)
  - [F. Deploy Using Kubernetes](#f-deploy-using-kubernetes)
    - [Deploy using Docker](#deploy-using-docker)
      - [Docker file of the central station:](#docker-file-of-the-central-station)
      - [Docker file of weather station:](#docker-file-of-weather-station)
      - [Docker file of rain detector:](#docker-file-of-rain-detector)
      - [docker-compose.yaml](#docker-composeyaml)
    - [Deploy using Kubernetes](#deploy-using-kubernetes)
  - [G. Profile Central Station Using JFR](#g-profile-central-station-using-jfr)
      - [Top 10 classes with the highest total memory:](#top-10-classes-with-the-highest-total-memory)
      - [GC pauses count and maximum pause duration:](#gc-pauses-count-and-maximum-pause-duration)
...
There are two variants of the weather station. The first is WeatherStation which runs a single weather station. The second is WeatherStations which has two additional classes:
...
A key-value store of the stations' statuses was implemented. The implementation is of BitCask Riak LSM as described in the paper. However, a hint file is generated and kept for each generated log file to speed up merging.
...
A [video](Central-Station/bitcask-directory-sample-video) showing the described behavior was recorded.

For speeding up the testing time we set the maximum size of the log file to be 100KB and the maximum number of logs to keep before merging to be 5.
...
[Sample BitCask LSM directory](Central-Station/bitcask)
...
- I.e, if station 1 sent a message and the status_timestamp of this message after conversion to date-time format is 2023-05-20 13:45, this record will be stored in  parquet_files/s1/s1\_\_2023-05-20\_\_13-45\_\_p0.parquet\
The Parquet file name is divided as follows:
  ...
  - pn: refers to part n ⇒ the Parquet file is created when writing a batch but there may be records that belong to the same file in the next batch Appending to parquet files in Java causes some problems, so another part of the file is created.
...
This is a sample parquet file "parquet-files\\s3\\s3\_\_2023-05-21\_\_10-25\_\_p0.parquet":
...
And this is the start of "parquet-files\\s3\\s3\_\_2023-05-21\_\_10-25\_\_p1.parquet":
...
[Sample Parquet Files](Central-Station/parquet-files)
...
![](readme/mappings.png)
6. Used the **count()** metric to visualize the percent of differentbattery statuses ("low", "medium", "high") and the donut-shaped chart sliced by the values of **"battery_status"**.
...
[Source Code](Weather-Statuses-Analysis/code): The python script can be found along with a notes text file to tell what to install in the container.
...
Following the provided and using Kompose, the k8s yaml files were generated using the command "kompose convert" which generates the services and deployments yaml files from the docker-compose file.
